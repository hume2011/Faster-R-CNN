...
def data_generator(dataset, config, shuffle=True, augment=False, augmentation=None,
    ...
    random_rois: If > 0 then generate proposals to be used to train the
                 network classifier and bbox heads. Useful if training
...
class FasterRCNN():
    ...
    def __init__(self, mode, config, model_dir):
        """
        mode: "training"或者"inference"。
        config: Config类的子类。
        model_dir: 存放日志和训练好的weights。
        """
    ...
    def build(self, mode, config):
        ...
        构建Faster R-CNN结构。
        input_shape: 输入图片的形状。
        mode: "training"或者"inference"，该模型的输入和输出相应的不同。
        """
        assert mode in ['training', 'inference']
        ...
        # 注意P6供RPN使用，不会用在classifier heads中
    ...
    def set_log_dir(self, model_path=None):
        ...
        self.log_dir = os.path.join(self.model_dir, "{}_{}_{:%Y%m%dT%H%M}".format(
            self.config.DATA_NAME.lower(), self.config.TASK_NAME.lower(), now))

        # Path to save after each epoch. Include placeholders that get filled by Keras.
        self.checkpoint_path = os.path.join(self.log_dir, "faster_rcnn_{}_{}_*epoch*.h5".format(
            self.config.DATA_NAME.lower(), self.config.TASK_NAME.lower()))
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        layers: Allows selecting wich layers to train. It can be:
            ...
            - One of these predefined values:
              heads: The RPN, classifier and bbox heads of the network
              all: All the layers
    ...
    def detect(self, images, verbose=0):
        ...
        for i, image in enumerate(images):
            final_rois, final_class_ids, final_scores =\
                self.unmold_detections(detections[i], image.shape,
...
def id_unuseful(img_folder):
    file_name = os.listdir(img_folder)
    for name in file_name:
        name = name.split('.')[0]
        if name == 'annotations':
            continue
        data = json.load(open(img_folder+'annotations/'+name+'.json'))
        if len(data['outputs']) == 0:
            os.rename(img_folder+'annotations/'+name+'.json',img_folder+'annotations/'+'no-box-'+name+'.json')
            os.rename(img_folder+name+'.jpg',img_folder+'no-box-'+name+'.jpg')
        elif len(data['outputs']['object']) == 0:
            os.rename(img_folder+'annotations/'+name+'.json',img_folder+'annotations/'+'no-box-'+name+'.json')
            os.rename(img_folder+name+'.jpg',img_folder+'no-box-'+name+'.jpg')

def split_to_train_val(path):
    
    imgdir_list = glob.glob(os.path.join(path,'*.jpg'))
    n_img = len(imgdir_list)
    n_val = int(n_img*0.2)
    seeds = np.random.choice(n_img,size=n_val,replace=False)
    
    for i in seeds:
        shutil.move(imgdir_list[i], os.path.join(path,'val/'))
        img_id = imgdir_list[i].split('/')[-1].split(".")[0]
        shutil.move(os.path.join(path,'annotations/')+str(img_id)+'.json',
                    os.path.join(path,'val/annotations/'))
        
    remain_imgdir_list = glob.glob(os.path.join(path,'*.jpg'))
    
    for i in range(len(remain_imgdir_list)):
        shutil.move(remain_imgdir_list[i], os.path.join(path,'train/'))
        img_id = remain_imgdir_list[i].split('/')[-1].split(".")[0]
        shutil.move(os.path.join(path,'annotations/')+str(img_id)+'.json', os.path.join(path,'train/annotations/'))
    os.rmdir(path+'annotations')#不要用removedirs，这会递归的删除整个路径

def change_path_in_anno(data_dir):
    
    train_anno_list = glob.glob(os.path.join(data_dir,'train/annotations/*.json'))
    for anno in train_anno_list:
        anno_data = json.load(open(anno))
        img_id = anno.split('/')[-1].split(".")[0]
        anno_data['path'] = data_dir+'train/'+str(img_id)+'.jpg'
        json.dump(anno_data,open(anno,'w'))
                    
    val_anno_list = glob.glob(os.path.join(data_dir,'val/annotations/*.json'))
    for anno in val_anno_list:
        anno_data = json.load(open(anno))
        img_id = anno.split('/')[-1].split(".")[0]
        anno_data['path'] = data_dir+'val/'+str(img_id)+'.jpg'
        json.dump(anno_data,open(anno,'w'))